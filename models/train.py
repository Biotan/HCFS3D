...
def train():
    with tf.get_default_graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            loss, sem_loss, disc_loss, l_var, l_dist, l_reg , l_inter_val = get_loss(pred_ins, labels_pl, pred_sem_label, pred_sem,
                                                                       sem_labels_pl)
            ...
            tf.summary.scalar('l_inter_val', l_inter_val)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        summary, step, _, loss_val, sem_loss_val, disc_loss_val, l_var_val, l_dist_val, l_reg_val, l_inter_val = sess.run(
            [ops['merged'], ops['step'], ops['train_op'], ops['loss'], ops['sem_loss'], ops['disc_loss'], ops['l_var'],
             ops['l_dist'], ops['l_reg'],ops['l_inter_val']],
            feed_dict=feed_dict)
        ...
        if batch_idx % 50 == 0:
            log_string(
                "loss: {:.2f}; sem_loss: {:.2f}; disc_loss: {:.2f}; l_var: {:.2f}; l_dist: {:.2f}; l_reg: {:.3f}; l_inter_val: {:.3f}.".format(
                    loss_val, sem_loss_val, disc_loss_val, l_var_val, l_dist_val,l_reg_val,  l_inter_val))
