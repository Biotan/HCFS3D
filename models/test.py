...
def test():
    with GRAPH.as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            pred_sem, pred_ins = get_model(pointclouds_pl, is_training_pl, NUM_CLASSES,FLAGS, GRAPH)
            pred_sem_softmax = tf.nn.softmax(pred_sem)
            pred_sem_label = tf.argmax(pred_sem_softmax, axis=2)

            loss, sem_loss, disc_loss, l_var, l_dist, l_reg = get_loss(pred_ins, labels_pl, pred_sem_label, pred_sem, sem_labels_pl,NUM_CLASSES)
